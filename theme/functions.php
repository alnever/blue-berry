...
function blue_berry_register_textdomain() {
  load_theme_textdomain('blue-berry', get_template_directory()."/languages");
}

add_action('after_setup_theme','blue_berry_register_textdomain');

// add theme support

function blue_berry_setup() {
...
add_action('after_setup_theme', 'blue_berry_setup');
...
function blue_berry_block_editor_styles() {
    wp_enqueue_style(
		'blue-berry-block-editor-styles',
		get_theme_file_uri( 'editor-style.css' ),
		false
	);
}

add_action( 'enqueue_block_editor_assets', 'blue_berry_block_editor_styles' );

// enqueue styles and scripts

function blue_berry_enqueue_styles_scripts() {
...
add_action('wp_enqueue_scripts', 'blue_berry_enqueue_styles_scripts');

// menus

function blue_berry_register_menus() {
  register_nav_menus(
    [
        'primary' => __('Primary Menu', 'blue-berry'),
        'secondary' => __('Secondary Menu', 'blue-berry'),
    ]
  );
}

add_action('init','blue_berry_register_menus');

function blue_berry_register_widget_areas() {
  register_sidebar([
    'name' => __('Right sidebar','blue-berry'),
    'id' => 'right-sidebar',
  ]);
  register_sidebar([
    'name' => __('Footer left sidebar','blue-berry'),
    'id' => 'footer-left-sidebar',
  ]);
  register_sidebar([
    'name' => __('Footer center sidebar','blue-berry'),
    'id' => 'footer-center-sidebar',
  ]);
  register_sidebar([
    'name' => __('Footer right sidebar','blue-berry'),
    'id' => 'footer-right-sidebar',
  ]);
  register_sidebar([
    'name' => __('Header sidebar','blue-berry'),
    'id' => 'header-sidebar',
  ]);
}

add_action('widgets_init', 'blue_berry_register_widget_areas');


// add footer text option

function blue_berry_register_footer_text_option ($wp_customize) {
...
add_action('customize_register', 'blue_berry_register_footer_text_option');

// add social buttons for footer

function blue_berry_sanitize_image_files($file, $setting) {
	$mimes = [
...
function blue_berry_register_footer_socials($wp_customize) {
    ...
    $wp_customize->add_setting('social_icon_1', [
        'default' => '',
        'transport' => 'refresh',
		'sanitize_callback' => 'blue_berry_sanitize_image_files',
    ]);
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'social_icon_3',[
        'label' => __('Social icon 3','blue-berry'),
        'section' => 'footer_socials_section',
        'setting' => 'social_icon_3',
    ]));
}

add_action('customize_register', 'blue_berry_register_footer_socials');
